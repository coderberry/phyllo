...
module Phyllo
  class Certifications1
    # Name of the certification
    attr_accessor :name

    # Identifier of the issued certificate
    attr_accessor :id
    ...
    def self.openapi_types
      {
        ...
        url: :String,
        authority: :String,
        time_period: :Certifications1TimePeriod
      }
    end
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Phyllo::Certifications1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Phyllo::Certifications1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [name, id, url, authority, time_period].hash
    end
