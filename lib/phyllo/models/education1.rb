  ...
  class Education1
    ...
    def self.openapi_types
      {
        ...
        description: :String,
        time_period: :Education1TimePeriod,
        activities: :String
      }
    end
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Phyllo::Education1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Phyllo::Education1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:field_study)
        self.field_study = attributes[:field_study]
      end
    ...
    def hash
      [degree, field_study, grade, school, description, time_period, activities].hash
    end
