...
module Phyllo
  # Media object to be published.
  class PublishContentMedia2
    # Media type of the media item being published.
    attr_accessor :media_type

    # Publicly accessible URL for the media to be uploaded.
    attr_accessor :source_media_url

    # Publicly accessible image URL to be used as the thumbnail to be uploaded.
    attr_accessor :source_thumbnail_url

    # Time offset in milliseconds within the video for the frame that should be used as the thumbnail (Instagram only).
    attr_accessor :thumbnail_offset

    # Platform specific fields come here.
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Phyllo::PublishContentMedia2` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Phyllo::PublishContentMedia2`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      self.media_type = if attributes.key?(:media_type)
        attributes[:media_type]
      end

      self.source_media_url = if attributes.key?(:source_media_url)
        attributes[:source_media_url]
      end

      if attributes.key?(:source_thumbnail_url)
        self.source_thumbnail_url = attributes[:source_thumbnail_url]
      end

      self.thumbnail_offset = if attributes.key?(:thumbnail_offset)
        attributes[:thumbnail_offset]
      else
        0
      end
    ...
    def list_invalid_properties
      ...
      if @source_media_url.nil?
        invalid_properties.push('invalid value for "source_media_url", source_media_url cannot be nil.')
      end
    ...
    def valid?
      ...
      media_type_validator = EnumAttributeValidator.new("String", ["IMAGE", "VIDEO"])
      return false unless media_type_validator.valid?(@media_type)
      return false if @source_media_url.nil?
      true
    end
    ...
    def ==(other)
      ...
      self.class == other.class &&
        ...
        source_media_url == other.source_media_url &&
        source_thumbnail_url == other.source_thumbnail_url &&
        thumbnail_offset == other.thumbnail_offset &&
    ...
    def hash
      [media_type, source_media_url, source_thumbnail_url, thumbnail_offset, additional_info].hash
    end
