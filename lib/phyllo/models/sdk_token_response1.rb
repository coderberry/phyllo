  ...
  class SDKTokenResponse1
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Phyllo::SDKTokenResponse1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Phyllo::SDKTokenResponse1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      self.sdk_token = if attributes.key?(:sdk_token)
        attributes[:sdk_token]
      end

      self.expires_at = if attributes.key?(:expires_at)
    ...
    def list_invalid_properties
      ...
      invalid_properties = []
      if @sdk_token.nil?
        invalid_properties.push('invalid value for "sdk_token", sdk_token cannot be nil.')
      end
