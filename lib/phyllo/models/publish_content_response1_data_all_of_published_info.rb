...
module Phyllo
  # Info of the published content item.
  class PublishContentResponse1DataAllOfPublishedInfo
    # Unique ID of the published content item.
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Phyllo::PublishContentResponse1DataAllOfPublishedInfo` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Phyllo::PublishContentResponse1DataAllOfPublishedInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      self.content_id = if attributes.key?(:content_id)
      ...
      self.media_url = if attributes.key?(:media_url)
        attributes[:media_url]
      end

      self.thumbnail_url = if attributes.key?(:thumbnail_url)
    ...
    def hash
      [content_id, url, media_url, thumbnail_url, published_at, platform_content_id].hash
    end
