...
module Phyllo
  class Content1Sponsored
    # Indicates if the content item is sponsored.
    attr_accessor :is_sponsored

    # Sponsor tag (can include mentions of the sponsor)
    attr_accessor :tags
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Phyllo::Content1Sponsored` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Phyllo::Content1Sponsored`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:is_sponsored)
        self.is_sponsored = attributes[:is_sponsored]
      end
    ...
    def ==(other)
      ...
      self.class == other.class &&
        is_sponsored == other.is_sponsored &&
        tags == other.tags
    end
