...
module Phyllo
  # Bank details to which the payouts are sent
  class CommercePayout1BankDetails
    # Bank name to which the payouts are sent
    attr_accessor :name

    # Last 4 characters of the bank account to which the payouts are sent
    attr_accessor :account_last_digits

    # Routing number of the bank account to which the payouts are sent
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Phyllo::CommercePayout1BankDetails` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Phyllo::CommercePayout1BankDetails`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:account_last_digits)
        self.account_last_digits = attributes[:account_last_digits]
      end
    ...
    def ==(other)
      ...
      self.class == other.class &&
        name == other.name &&
        account_last_digits == other.account_last_digits &&
        account_routing_number == other.account_routing_number
    end
    ...
    def hash
      [name, account_last_digits, account_routing_number].hash
    end
