...
module Phyllo
  # Webhook object to send notification on the registered webhook URL.
  class WebhookPayload1
    # Name of the event for which webhook is being triggered
    attr_accessor :event

    # Description of the event
    attr_accessor :name

    # Corresponding data payload, [read about the events and corresponding payloads here](../../../API/webhook-events)
    attr_accessor :data
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Phyllo::WebhookPayload1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Phyllo::WebhookPayload1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      self.event = if attributes.key?(:event)
        attributes[:event]
      end
    ...
    def event=(event)
      validator = EnumAttributeValidator.new("String", ["ACCOUNTS.CONNECTED", "ACCOUNTS.DISCONNECTED", "PROFILES.ADDED", "PROFILES.UPDATED", "PROFILES_AUDIENCE.ADDED", "PROFILES_AUDIENCE.UPDATED", "CONTENTS.ADDED", "CONTENTS.UPDATED", "CONTENTS_COMMENTS.ADDED", "CONTENTS_COMMENTS.UPDATED", "CONTENT-GROUPS.ADDED", "CONTENT-GROUPS.UPDATED", "TRANSACTIONS.ADDED", "TRANSACTIONS.UPDATED", "PAYOUTS.ADDED", "PAYOUTS.UPDATED", "BALANCES.ADDED", "BALANCES.UPDATED", "CONTENTS.PUBLISH_SUCCESS", "CONTENTS.PUBLISH_READY", "CONTENTS.PUBLISH_FAILURE", "SESSION.EXPIRED", "ACTIVITY-ARTISTS.ADDED", "ACTIVITY-ARTISTS.UPDATED", "ACTIVITY-CONTENTS.ADDED", "ACTIVITY-CONTENTS.UPDATED"])
