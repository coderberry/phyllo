...
module Phyllo
  class SocialTransaction1
    ...
    # \"Cost per thousand impressions\", where impressions are the display units, as paid by the platform to show ads on the profile
    attr_accessor :cpm
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Phyllo::SocialTransaction1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Phyllo::SocialTransaction1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      self.cpm = if attributes.key?(:cpm)
        attributes[:cpm]
      end
      ...
      self.transaction_at = if attributes.key?(:transaction_at)
        attributes[:transaction_at]
      end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new("String", ["AD", "SUBSCRIPTION", "ADDITIONAL"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new("String", ["AD", "SUBSCRIPTION", "ADDITIONAL"])
    ...
    def hash
      [id, created_at, updated_at, user, account, work_platform, amount, type, cpm, currency, external_id, transaction_at, platform_profile_id, platform_profile_name].hash
    end
