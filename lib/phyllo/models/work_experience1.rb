...
module Phyllo
  class WorkExperience1
    ...
    def self.openapi_types
      {
        :'title' => :'String',
        :'company' => :'WorkExperience1Company',
        :'description' => :'String',
        :'time_period' => :'WorkExperience1TimePeriod',
        :'location' => :'WorkExperience1Location'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Phyllo::WorkExperience1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Phyllo::WorkExperience1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [title, company, description, time_period, location].hash
    end
