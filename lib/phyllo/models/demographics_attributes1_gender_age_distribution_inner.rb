...
module Phyllo
  class DemographicsAttributes1GenderAgeDistributionInner
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Phyllo::DemographicsAttributes1GenderAgeDistributionInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Phyllo::DemographicsAttributes1GenderAgeDistributionInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      self.age_range = if attributes.key?(:age_range)
    ...
    def list_invalid_properties
      ...
      if @age_range.nil?
        invalid_properties.push('invalid value for "age_range", age_range cannot be nil.')
      end
